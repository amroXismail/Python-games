...
def handle_bullets(player1rect,player2rect,player1_bullets,player2_bullets,Lplat1_rect,Rplat1_rect,blue_box_rect):
...
def main():
    ...
    while(run):
        ...
        pressed_key=pyg.key.get_pressed()
        #game events
        if pressed_key[pyg.K_q] or (bool(joysticks) and joysticks[0].get_button(7)):
        ...
        if pressed_key[pyg.K_a] or (bool(joysticks) and joysticks[0].get_axis(0)<0):
            ...
            if not one_jetpacking:
                player1.animate()
        if pressed_key[pyg.K_w] or (bool(joysticks) and joysticks[0].get_button(6)):
        ...
        if pressed_key[pyg.K_LSHIFT] or (bool(joysticks) and joysticks[1].get_button(7)):
        ...
        if pressed_key[pyg.K_LEFT] or (bool(joysticks) and joysticks[1].get_axis(0)<0):
            ...
            if not two_jetpacking:
                player2.animate()
        if pressed_key[pyg.K_UP] or (bool(joysticks) and joysticks[1].get_button(6)):
        ...
        for event in pyg.event.get():
            ...
            if event.type == pyg.JOYBUTTONDOWN or event.type == pyg.KEYDOWN:
                if (bool(joysticks) and joysticks[0].get_button(3)) or (event.type == pyg.KEYDOWN and event.key == pyg.K_r):
                    ...
                    player1.reload()
                if (bool(joysticks) and joysticks[0].get_button(7)) or (event.type == pyg.KEYDOWN and event.key == pyg.K_q):
                ...
                if (bool(joysticks) and joysticks[1].get_button(3)) or  (event.type == pyg.KEYDOWN and event.key == pyg.K_m):
                    ...
                    player2.reload()
                if (bool(joysticks) and joysticks[1].get_button(7)) or  (event.type == pyg.KEYDOWN and event.key == pyg.K_LSHIFT):
            ...
            if event.type == pyg.JOYBUTTONUP or event.type == pyg.KEYUP:
                if (bool(joysticks) and not joysticks[0].get_button(7)) or (event.type == pyg.KEYUP and event.key == pyg.K_q):
                    if AUTO_FIRE_SOUND1.get_num_channels() > 0:
                        AUTO_FIRE_SOUND1.stop()
                ...
                if (bool(joysticks) and not joysticks[1].get_button(7)) or  (event.type == pyg.KEYUP and event.key == pyg.K_LSHIFT):
                    if AUTO_FIRE_SOUND2.get_num_channels() > 0:
                        AUTO_FIRE_SOUND2.stop()
        ...
        if (player1.pos[1]+ICE_MONSTER_AUTO_SIZE[1] == HEIGHT - PLATFORM1.get_height() and (-ICE_MONSTER_AUTO_SIZE[0]*2/5 <= player1.pos[0] <= PLATFORM1.get_width()-ICE_MONSTER_AUTO_SIZE[0]*1/5) ) or one_jetpacking or one_jumping:
            one_falling = False
            one_fall_speed = FALL_SPEED
        else:
            one_falling=True
        ...
        if (player2.pos[1]+ICE_MONSTER_AUTO_SIZE[1] == HEIGHT - PLATFORM1.get_height() and (WIDTH - PLATFORM1.get_width()<= player2.pos[0] + player2.current_surf.get_width() - JET_WIDTH <= WIDTH + JET_WIDTH*2 ) ) or two_jetpacking or two_jumping:
            two_falling = False
        ...
        handle_bullets(player1rect,player2rect,player1_bullets,player2_bullets,Lplat1_rect,Rplat1_rect,blue_box_rect)
